			...
			<div class="row justify-content-center mb-5">
				...
				<div class="col-lg-6 mb-4">
					<div id="carouselExampleIndicators"
						class="carousel slide rounded shadow" data-bs-ride="carousel">
						...
						<div class="carousel-inner">
							...
							<div class="carousel-item active">
								<img src="${ pd.firstUrl }" class="d-block w-100" alt="..." />
							</div>
							<div class="carousel-item">
								<img src="${pd.secondUrl }" class="d-block w-100" alt="..." />
							</div>
							<div class="carousel-item">
								<img src="${pd.thirdUrl }" class="d-block w-100" alt="..." />
							</div>
						</div>
							...
							<div class="card-body">
								<div class="d-flex justify-content-between align-items-start"
									...
									<h2 class="card-title mb-5">
										${ pd.placeName } <span id="iscoreBedge"
											class="badge bg-secondary ms-2" style="font-size: 13px;">${ pd.avgIScore }
										</span>
									</h2>
									<i id="favoriteHeart" class="bi-heart" style="color: red;"
										data-favorite="${pd.userFavorite}"></i>

								</div>

								<p class="card-text mb-3">
									<i class="fas fa-map-marker-alt me-2" style="color: #4a6fa5"></i>
									${ pd.address }
								</p>
								<p class="card-text mb-3">
									<i class="far fa-clock me-2" style="color: #4a6fa5"></i> ${ pd.operatingHours }
								</p>
								<p class="card-text mb-3">
									<i class="fas fa-info-circle me-2" style="color: #4a6fa5"></i>
						...
						<form id="reviewForm">
							<div class="btn-group mb-3" role="group"
								...
								<input type="checkbox" class="btn-check" id="btncheck1"
									name="features" value="주차가능" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btncheck1"
									data-parkingAvailable="${pd.parkingAvailable}">주차가능</label>
								<input type="checkbox" class="btn-check" id="btncheck2"
									name="features" value="경치좋은" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btncheck2"
									data-view="${ pd.view }">경치좋은</label> <input
									type="checkbox" class="btn-check" id="btncheck3"
									name="features" value="무료입장" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btncheck3"
									data-freeEntry="${pd.freeEntry}">무료입장</label>
								<input type="checkbox" class="btn-check" id="btncheck4"
									name="features" value="야경" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btncheck4"
									data-nightView="${pd.nightView}">야경</label>
								<input type="checkbox" class="btn-check" id="btncheck5"
									name="features" value="교통원활" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btncheck5"
									data-easyTransport="${pd.easyTransport}">교통원활</label>
							</div>
							<h3 class="mb-0">아이난이도</h3>
							<div class="btn-group mb-3" role="group"
								aria-label="Basic radio toggle button group">
								<input type="radio" class="btn-check" name="difficulty"
									id="btnradio1" value="상" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btnradio1">상</label>
								<input type="radio" class="btn-check" name="difficulty"
									id="btnradio2" value="중" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btnradio2">중</label>
								<input type="radio" class="btn-check" name="difficulty"
									id="btnradio3" value="하" autocomplete="off" />
								<label class="btn btn-outline-primary" for="btnradio3">하</label>
							</div>
							<textarea class="form-control mb-3" id="reviewText" rows="4"
								placeholder="리뷰를 작성해주세요">${ pd.review }</textarea>
							<div id="reviewButtonsContainer">
								<c:choose>
									<c:when test="${empty pd.review}">
										<button id="createReviewBtn" type="button"
											class="btn btn-primary">작성완료</button>
									</c:when>
									<c:otherwise>
										<button id="updateReviewBtn" type="button"
											class="btn btn-warning">수정하기</button>
										<button id="deleteReviewBtn" type="button"
			...
			<h2 class="mb-4 d-flex justify-content-between align-items-center">
				...
				<div class="dropdown">
					<button class="btn dropdown-toggle" type="button"
						id="sortDropdownButton" data-bs-toggle="dropdown"
						aria-expanded="false">좋아요 많은순</button>
					<ul class="dropdown-menu" aria-labelledby="sortDropdownButton">
						<li>
							<button class="btn dropdown-item1 active"
								data-sort="modifiedAtDESC">최신순</button>
						</li>
						<li>

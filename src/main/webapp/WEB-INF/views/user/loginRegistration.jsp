...
<body>
		...
		<div class="container">
			...
			<div class="formBx">
				<div class="form signinform">
					...
					<form action="${loginUrl}" id="loginForm" method="post">
						...
						<input type="text" class="form-control" id="loginId" name="userId"
							placeholder="아이디를 입력해 주세요" value="${userId}" /> <input
							type="password" class="form-control" id="loginPassword"
							name="password" placeholder="비밀번호를 입력해 주세요" />
						<!-- 로그인 체크 메시지 -->
						<div id="login-check-message" class="invalid-feedback"
							style="display: none"></div>
						<input type="submit" class="form-control" id="loginBtn"
							value="로그인" />
						<div class="find">
							<a type="button" class="btnIdPw" data-bs-toggle="modal"
								data-bs-target="#findUserIdModal">아이디를
								잊으셨나요?</a> <span> | </span> <a type="button" class="btnIdPw"
								data-bs-toggle="modal" data-bs-target="#findPasswordModal">비밀번호를
								잊으셨나요?</a>
						</div>
					</form>
				</div>
				<div class="form signupform">
					...
					<form action="${registerUrl}" id="registerForm" method="post"
						enctype="multipart/form-data">
						<h3>회원가입</h3>
						<div class="image-upload d-flex justify-content-center">
							<label for="profile-input"> <img src="${defaultImageUrl}"
								class="rounded-circle mx-auto d-block" type="button"
								id="register-profileimg" />
								<div class="upload-icon">
									<ion-icon name="camera-outline"></ion-icon>
								</div> <input class="d-none" id="profile-input" name="file"
						...
						<div id="id-check-message" class="invalid-feedback"
							style="display: none"></div>

						<input type="password" class="form-control" placeholder="비밀번호"
							id="password" name="password"
							oninput="validateInput('password', this.value)" />
						<div id="password-check-message" class="invalid-feedback"
							style="display: none"></div>

						<input type="password" class="form-control" placeholder="비밀번호 확인"
							id="password-confirm"
							oninput="validateInput('password-confirm', this.value)" />
						<div id="password-confirm-check-message" class="invalid-feedback"
							style="display: none"></div>

						<input type="text" class="form-control" placeholder="닉네임"
							id="nickName" name="nickName"
							oninput="validateInput('nickName', this.value)" />
						<div id="nickname-check-message" class="invalid-feedback"
							style="display: none"></div>

						<input type="email" class="form-control" placeholder="이메일"
							id="email" name="email"
							oninput="validateInput('email', this.value)" />
						<div id="email-check-message" class="invalid-feedback"
							style="display: none"></div>

						<div class="d-flex align-items-baseline" id="phone-input">
							<input type="text" class="form-control" placeholder="010"
								id="phone1" name="phone1" maxlength="3"
								oninput="validateInput('phone', this.value)" />
							<span>-</span> <input type="text" class="form-control"
								placeholder="1234" id="phone2" name="phone2" maxlength="4"
								oninput="validateInput('phone', this.value)" />
							<span>-</span> <input type="text" class="form-control"
								placeholder="5678" id="phone3" name="phone3" maxlength="4"
								oninput="validateInput('phone', this.value)" />
						</div>
						<div id="phone-check-message" class="invalid-feedback"
							style="display: none"></div>
						<div id="register-check-message" style="display: none"></div>
						<input type="submit" id="registerBtn" name="registerBtn"
							value="회원가입" />
					</form>
				</div>
			</div>
	...
	<!-- 아이디 찾기 모달 -->
	<div class="modal fade" id="findUserIdModal" tabindex="-1"
		aria-labelledby="findUserIdModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body" id="findUserIdModalBody">
					<!-- 아이디 찾기 폼 내용 -->
					<input type="email" name="email" id="emailForFindUserId"
						class="form-control" placeholder="이메일을 입력해 주세요" /> <input
						type="nickName" name="nickName" id="nickNameForFindUserId"
						class="form-control mt-2" placeholder="닉네임을 입력해 주세요" />
					<label id="findUserIdMessage" class="invalid-feedback"
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body" id="findPasswordModalBody">
					<!-- 비밀번호 찾기 폼 내용 -->
					<input type="text" id="userIdForFindPassword" class="form-control"
						placeholder="아이디를 입력해 주세요" /> <input type="email"
						id="emailForFindPassword" class="form-control mt-2"
						placeholder="이메일을 입력해 주세요" /> <input type="nickName"
						id="nickNameForFindPassword" class="form-control mt-2"
						placeholder="닉네임을 입력해 주세요" /> <label
						id="findPasswordMessage" class="invalid-feedback"
						style="display: none"></label>
				</div>
